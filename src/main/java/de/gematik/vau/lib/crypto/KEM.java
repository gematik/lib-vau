...
public class KEM {
  ...
  public static KdfMessage decapsulateMessages(
    ...
    byte[] ecdhSharedSecret =
        EllipticCurve.getSharedSecret(
            ecdhPublicKeySender, privateKeys.getEccKeyPair().getPrivate());

    byte[] sharedSecretClient =
        KyberEncoding.pqcGenerateDecryptionKey(
            privateKeys.getKyberKeyPair().getPrivate(), ciphertext.getKyberCt());
  ...
  public static KdfKey1 kdf(KdfMessage message) {
    if (message == null) {
      ...
      List<byte[]> byteArrays =
          kdf(ArrayUtils.addAll(message.getEcdhSharedSecret(), message.getKyberSharedSecret()), 2);
  ...
  public static KdfKey2 kdf(KdfMessage message1, KdfMessage message2) {
    ...
    List<byte[]> byteArrays =
        kdf(
            ArrayUtils.addAll(
                ArrayUtils.addAll(message1.getEcdhSharedSecret(), message1.getKyberSharedSecret()),
                ArrayUtils.addAll(message2.getEcdhSharedSecret(), message2.getKyberSharedSecret())),
            5);
    return new KdfKey2(
        byteArrays.get(0),
        byteArrays.get(1),
