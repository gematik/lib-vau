...
public class VauServerStateMachine extends AbstractVauStateMachine {
  ...
  private byte[] receiveMessage1(VauMessage1 vauMessage1, byte[] message1Encoded) {
    ...
    kemResult1 =
        KEM.encapsulateMessage(
            vauMessage1.getEcdhPublicKey().toEcPublicKey(), vauMessage1.toKyberPublicKey());
    if (log.isTraceEnabled()) {
      log.trace(
          "ecdh_shared_secret: (hexdump) {}", Hex.toHexString(kemResult1.getEcdhSharedSecret()));
      log.trace(
          "Kyber768_shared_secret: (hexdump) {}",
          Hex.toHexString(kemResult1.getKyberSharedSecret()));
    }
    ...
    byte[] aeadCiphertextMessage2 =
        KEM.encryptAead(kdfServerKey1.getServerToClient(), encodedSignedPublicVauKeys);
    VauMessage2 message2 =
        new VauMessage2(kemResult1.getEcdhCt(), kemResult1.getKyberCt(), aeadCiphertextMessage2);
  ...
  private byte[] receiveMessage3(VauMessage3 vauMessage3, byte[] message3Encoded) {
    try {
      ...
      byte[] clientTranscriptHash =
          KEM.decryptAead(
              serverKey2.getClientToServerKeyConfirmation(),
              vauMessage3.getAeadCtKeyKonfirmation());
      ...
      byte[] aeadCiphertextMessage4KeyKonfirmation =
          KEM.encryptAead(serverKey2.getServerToClientKeyConfirmation(), transcriptServerHash);
    ...
    if (!Arrays.equals(serverKey2.getKeyId(), keyId)) {
      throw new IllegalArgumentException(
          "Key ID in the header "
              ...
              + " does not equals "
              + Hex.toHexString(serverKey2.getKeyId())
  ...
  private void verifyClientMessageIsWellFormed(VauMessage1 vauMessage1) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Kyber Public Key Bytes in VAU Message 1 are not well formed.", e);
    }
  }
}
