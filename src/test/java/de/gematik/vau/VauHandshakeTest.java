...
class VauHandshakeTest {
  ...
  void testHandshake() {
    ...
    final VauPublicKeys serverVauKeys =
        new VauPublicKeys(serverVauKeyPair, "VAU Server Keys", Duration.ofDays(30));
    ...
    assertThat(serverVauKeyPair.getKyberKeyPair().getPublic().getAlgorithm())
        .isEqualTo("ML-KEM-768");
    assertThat(serverVauKeyPair.getKyberKeyPair().getPrivate().getAlgorithm())
        .isEqualTo("ML-KEM-768");
    ...
    assertThat(client.getKdfClientKey1().getServerToClient()).isEqualTo(server.getS2c());
    assertThat(client.getKdfClientKey1().getClientToServer()).isEqualTo(server.getC2s());
    ...
    bundleInHttpRequestAndWriteToFile(
        ...
        Pair.of(
            "VAU-nonPU-Tracing",
            Base64.toBase64String(server.getServerKey2().getClientToServerAppData())
                + " "
                + Base64.toBase64String(server.getServerKey2().getServerToClientAppData())
                + "\n"));
    bundleInHttpResponseAndWriteToFile(encryptedServerVauMessage);

    Files.write(
        Path.of("target/serverEcc.pem"), writeKeyPair(serverVauKeyPair.getEccKeyPair()).getBytes());
    Files.write(
        Path.of("target/serverKyber.pem"),
        writeKeyPair(serverVauKeyPair.getKyberKeyPair()).getBytes());
  }
  ...
  private void bundleInHttpRequestAndWriteToFile(
      String path, byte[] payload, Pair<String, String>... additionalHeader) {
  ...
  @SneakyThrows
  private void bundleInHttpResponseAndWriteToFile(
      byte[] payload, Pair<String, String>... additionalHeader) {
    String additionalHeaders =
        Stream.of(additionalHeader)
            .map(p -> p.getLeft() + ": " + p.getRight())
            .collect(Collectors.joining("\r\n"));
